...
from .models import Person, Investor, Company, Message, Picture
    ...
    if request.method == 'POST':
        ...
        if val == "1":
            ...
            if create_investor(request):
                ...
                return HttpResponseRedirect("/Kahi/")
            else:
...
def create_company(request):
    ...
    if not User.objects.filter(username=company_username).exists():
        ...
        company = Company.objects.create(company_time = company_time,company_name=company_name, company_username=company_username,
...
def investor(request):
    if request.user.is_authenticated():
        if is_investor(request):
            ...
            template = loader.get_template("company_home.html")
...
def is_investor(request):
    try:
        is_investor = Investor.objects.get(username=request.user.username)
    except:
        is_investor = None

    if not is_investor == None:
        return True
    return False
...
def company_page(request):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST':
            ...
            newpic = Picture(picfile=request.FILES['picfile'], owner=company.company_username, timestamp= datetime.now(), investor= investor.username)
            newpic.save()
        ...
        pictures = Picture.objects.filter(owner=company.company_username)
        return render_to_response('company_page.html', {'pictures': pictures, 'form': form, 'company': company},
...
def edit_company(request):
    if request.user.is_authenticated():
        if request.method == 'POST':
            ...
            company.save(
                update_fields=['company_update', 'company_ceo', 'company_email', 'company_state', 'company_country', 'company_phone',
                               'company_sector', 'company_industry'])
            return HttpResponseRedirect("/Kahi/home")
        else:
            company = Company.objects.get(company_username=request.user.username)
            context = {
                'company': company,
...
def compose_message(request):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST':
            ...
            return HttpResponseRedirect("/Kahi/messages_home")
        else:
...
def add_project(request):
    if request.user.is_authenticated():
        if request.method == 'POST':
            company = Company.objects.get(company_username=request.user.username)
            company.company_update = datetime.now()
            company.company_project = request.POST.getlist('project')
            company.save(
            update_fields=['company_update','company_project'])
            return HttpResponseRedirect("/Kahi/home")
        else:
            company = Company.objects.get(company_username=request.user.username)
            context = {
