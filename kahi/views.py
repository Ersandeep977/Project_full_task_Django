...
def edit_report(request):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST':
            ...
            return HttpResponseRedirect("/Kahi/company_page/?value2=" + value2 + "&value=" + value1)
        else:
...
def remove_self_from_group(request):
    if request.user.is_authenticated():
        ...
        return HttpResponseRedirect("/Kahi/groups_home/")
    else:
        return go_to_login()
...
def remove_project1(request):
    if request.user.is_authenticated():
        ...
        report = Project.objects.get(project_id=value1, project=value2).delete();
        return HttpResponseRedirect("/Kahi/company_page/?value2=" + value1 + "&value=" + value3)
