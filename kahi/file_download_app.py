...
def check_user_exists(username, password):
    ...
    r = requests.post(url + 'Kahi/login_accepted/', data=json.dumps(payload), headers=headers)
...
def get_report_data():
    ...
    tempUrl = url + "Kahi/app_report_details/" + str(pk) + "/"
...
def get_report_files(report_id):
    ...
    headers = {'content-type': 'application/json'}
    r = requests.post(url + 'Kahi/get_pictures/', data=json.dumps(payload), headers=headers)
    data = r.json()
...
def list_all_reports():
    temp_url = url + "Kahi/list_all_reports/"
    payload = {"user": user}
...
def encrypt_file(file_name):
    ...
    arc = ARC4.new("Kahi")
    with open(file_name, 'rb') as original_file, open(file_name + ".Kahi", 'wb') as encoded_file:
        for line in original_file:
            encoded_file.write(arc.encrypt(line))
    print("File encrypted! Saved as " + file_name + ".Kahi")
